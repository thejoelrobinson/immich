...
const walmartLogo = `<?xml version="1.0" encoding="iso-8859-1"?>
<!-- Walmart Spark Logo -->
...
async function replaceImmichLogo() {
  try {
    const logoPath = resolve(__dirname, '../node_modules/@immich/ui/dist/assets/immich-logo.svg');

    // Check if the file exists before trying to write
    try {
      await fs.access(logoPath);
    } catch {
      console.log('Logo file not found, skipping replacement');
      return;
    }

    await fs.writeFile(logoPath, walmartLogo, 'utf8');
    console.log('✓ Replaced Immich logo with Walmart Spark logo');
  } catch (error) {
    console.error('Failed to replace logo:', error);
    // Don't fail the build if logo replacement fails
  }
}
